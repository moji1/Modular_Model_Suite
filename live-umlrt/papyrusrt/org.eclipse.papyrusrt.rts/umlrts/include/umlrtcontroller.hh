...
public:
    ...
    typedef enum {
        ...
        E_SAPREG_SERVICE_INV,   // attempt to register a SAP port with an invalid service name.
...
#define UMLRTCONTROLLER_ERROR_CODE_TO_STRING  {\
    ...
    "SAP register failed - invalid service name", \
    ...
    // Abort the controller (or all controllers). This release always aborts all.
    void abort ( ) { enqueueAbortAllControllers(); }
    void abortAllControllers ( ) { enqueueAbortAllControllers(); }
    ...
    void deallocateSlotResources ( UMLRTSlot * slot, UMLRTCapsule * deletedCapsule, bool isDestroy );
    ...
    // Enqueue an exit command.
    void enqueueExit ( void * exitValue );

    // Enqueue an exit command to all controllers.
    void enqueueExitAllControllers ( void * exitValue );
    ...
    void enqueueDeport (  UMLRTSlot * slot, UMLRTCapsule * capsule );
    ...
    void enqueueIncarnate ( UMLRTCapsule * capsule, const UMLRTCommsPort * srcPort, const UMLRTObject_class * type, const void * userData );

    // Normal (non-error) exit for this controller.
    void exit ( void * exitValue )  { enqueueExitAllControllers(exitValue); }

    // Exit all controllers.
    void exitAllControllers ( void * exitValue ) { enqueueExitAllControllers(exitValue); }
...
private:
    ...
    // Used to match controller commands that should be deallocated from message queues during DEPORT/DESTROY.
    typedef struct
    {
        UMLRTSlot * slot;
    ...
    } t_message_match_criteria;

    // Callback to purge messages for a condemned slot.
    static bool deallocateMsgMatchCompare ( UMLRTMessage * msg, t_message_match_criteria * criteria );
    ...
    void executeCommand ( UMLRTMessage * msg );
    ...
    // Output one queued message.
    void debugOutputMessage ( UMLRTMessage * msg );

    // Output queued messages.
    void debugOutputMessages ( );
